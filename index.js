...
const validateBook = (req, res, next) => {
  ...
  if (missingFields){
    next(missingFields);
    //this below line can also be done like one of these - - in these cases the next line of code would be next();, without 'else':
    // return res.status(400).json({error: missingFields}); 
    // or return res.json({error: missingFields}); 
  } else {
    next();
  }  
};
